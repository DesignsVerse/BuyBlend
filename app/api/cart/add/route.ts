...
export async function POST(req: Request) {
  ...
  try {
    const { userId, sessionId, productId, productName,  variantId, quantity, unitPrice, currency } = await req.json();

    if (!productId || !variantId || !productName || !quantity || !unitPrice || !currency) {
