    ...
    return (
      <motion.div
        ...
        whileHover={{
          ...
          y: -10, // Lift up by 10px
          transition: { type: "spring", stiffness: 300, damping: 20 }, // Smooth spring animation
        }}
        onHoverStart={() => shadow.set("0 8px 16px rgba(0, 0, 0, 0.1)")} // Stronger shadow on hover
        onHoverEnd={() => shadow.set("0 4px 8px rgba(0, 0, 0, 0.05)")} // Default shadow
        style={{ boxShadow: springShadow }} // Apply animated shadow
      >
        <Link href={`/collection/product/${productSlug}`} className="relative bg-white dark:bg-black shadow-md overflow-hidden border flex flex-col h-full">
          {/* Media */}
          ...
          <div className="p-4 pt-0">
            {onSelectForCombo ? (
              ...
              <AddToCartButton
                ...
                onClick={(e) => e.stopPropagation()} // Prevent click from bubbling to Link
              />
            )}
          </div>
        </Link>
