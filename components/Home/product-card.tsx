...
export function ProductCard({ product, onSelectForCombo, isSelected }: ProductCardProps) {
  ...
  const imageArray = useMemo(
    () => (product.media || []).filter((m: any) => m._type === "image"),
    [product.media]
  )
  ...
  useEffect(() => {
    if (!isHovered) setCurrentMediaIndex(0)
  }, [isHovered])

  const imageVariants = {
    enter: (direction: number) => ({ x: direction === 0 ? 300 : -300, opacity: 0, scale: 1.05 }),
  ...
  return (
    <motion.div
      ...
      onHoverStart={() => { shadow.set("0 8px 16px rgba(0, 0, 0, 0.1)"); setIsHovered(true) }}
      ...
      <div className="relative bg-white dark:bg-black shadow-md overflow-hidden border flex flex-col h-full   ">
        ...
        <div className="relative">
              ...
              <AnimatePresence initial={false} custom={direction}>
                ...
                >
                  {currentImage ? (
                    <Image
                      src={currentImageUrl}
                      alt={(currentImage as any).alt || product.name}
                      fill
                      className={`object-cover ${isMediaLoaded ? "opacity-100" : "opacity-0"}`}
                      onLoad={() => setIsMediaLoaded(true)}
                      priority={!!(product as any).featured && currentMediaIndex === 0}
                    />
                  ) : (
          ...
          <button
            ...
            className={`absolute top-2 right-2 h-8 w-8 sm:h-9 sm:w-9 rounded-full flex items-center justify-center shadow-md z-20 transition-all duration-300 ${
              isWishlisted ? "bg-red-500 text-white" : "bg-white/80 text-gray-700 hover:bg-white hover:scale-110"
            }`}
            style={{ zIndex: 20 }}
          >
          ...
          {/* Rating */}
          <div
            className="absolute left-2 bottom-2 px-2 py-1 rounded-md bg-white/95 flex items-center gap-1 shadow text-[10px] sm:text-xs font-semibold z-10 min-w-[40px]"
        ...
        <div className="p-3 sm:p-4 flex flex-col gap-1 flex-grow">
          <Link href={`/collection/product/${productSlug}`} className="block">
            ...
            {product.description && (
              <p className="mt-1 text-[11px] sm:text-xs text-gray-600 dark:text-gray-400 line-clamp-1">
                {product.description}
              </p>
            )}
          </Link>
          ...
          <div className="flex items-center gap-1.5 mt-1.5 sm:mt-2">
            <span className="text-[14px] sm:text-base font-bold">
